...
const mvaeModel = new mvae.MusicVAE(modelCheckpoint);
const midimeModel = new mvae.MidiMe({epochs: 100});
let i = 1;
const embeddingsFileName = 'embeddings_midime.tsv';
const metaDataFileName = 'metadata_midime.tsv';

init().then(() => console.log('Finished!'));

async function init() {
    console.log('Starting...');
    await mvaeModel.initialize();
    await midimeModel.initialize();
    await train();
...
async function train() {
    // Define Files to train MidiMe on
    const midiFilesUrl = [].concat(midiFilesCatchy, midiFilesDark, midiFilesEDM, midiFilesEmotional, midiFilesPop, midiFilesRnB);
    console.log(midiFilesUrl);
    const melodies = [];
    ...
    const z = await mvaeModel.encode(chunks);  // shape of z is [chunks, 256]

    // 2. Use that z as input to train MidiMe.
    const losses = [];
    console.log('Training started...');
    // tslint:disable-next-line:no-any
    // await midimeModel.train(z, async (epoch, logs) => {
    //     losses.push(logs.total);
    // });
    await midimeModel.train(z);
...
async function createEmbeddingFileFor(category, midiFilesUrl) {
    ...
    // Get for every chunk a MusicVAE z value
    const z = await mvaeModel.encode(chunks);  // shape of z is [chunks, 256]
    const zMidiMe = await midimeModel.encode(z); // MidiMe Zs for category snippets
    const attributeVectorZMean = zMidiMe.mean(0, true); // mean of all chunks. Shape: [1, 256]
    ...
    let csvMetaData = '';
    zMidiMe.arraySync().forEach(function(rowArray, index) {
